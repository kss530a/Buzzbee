...
with tf.name_scope('input_data') as scope:
    X = tf.placeholder(tf.float32, [None, data_dim], name="X_input")
...
with tf.name_scope('rnn_layer') as scope:
    ...
    cell = tf.contrib.rnn.MultiRNNCell([cell] * rnn_layer_size, state_is_tuple=True)
...
with tf.name_scope('FC_layer1') as scope:
    W1 = tf.Variable(tf.random_normal([data_dim, fc_hidden_dim]), name='weight1')
    B1 = tf.Variable(tf.random_normal([fc_hidden_dim]), name='bias1')
    L1 = tf.nn.relu(tf.matmul(outputs, W1) + B1)
...
with tf.name_scope('FC_layer2') as scope:
    ...
    B2 = tf.Variable(tf.random_normal([fc_hidden_dim]), name='bias2')
